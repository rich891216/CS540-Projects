...
hidden = 1 / (1 + np.exp(-(np.matmul(new_x, W1)+b1)))
new_a = 1 / (1 + np.exp(-(np.matmul(W2, np.transpose(hidden)) + b2)))
new_a = new_a.ravel()

###for data in range(200):
#    new_a.append(1 / (1 + np.exp(-(np.matmul(W2, hidden) + b2))))
activation = [round(num, 2) for num in new_a]
new_id = []
